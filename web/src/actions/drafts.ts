  ...
  const res = await getNNDB()
    ...
    .execute(async (tx) => {
      ...
      await tx
        ...
        .where((eb) =>
          eb("id", "not in", eb.selectFrom("editions").select("article_id")),
        )
        .execute();
...
async function createDraft(auth: z.input<typeof authSchema>) {
  ...
  return await getNNDB()
    .with("new_article", (db) =>
      db.insertInto("articles").values({ author_id: authorId }).returning("id"),
    )
    .insertInto("drafts")
    .values((eb) => ({
...
async function publishDraft(
  ...
  const res = await getNNDB()
    ...
    .execute(async (tx) => {
      ...
      await tx.deleteFrom("drafts").where("id", "=", draftId).execute();
